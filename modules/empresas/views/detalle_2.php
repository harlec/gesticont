...
try {
    ...
    $stmtV = $db->prepare("SELECT COUNT(*) as cant, COALESCE(SUM(total),0) as total FROM registro_ventas WHERE empresa_id = ? AND DATE_FORMAT(fecha_emision,'%Y%m') = DATE_FORMAT(CURDATE(),'%Y%m')");
    $stmtV->execute([$empresa['id']]);
    $ultimasVentas = $stmtV->fetch(PDO::FETCH_ASSOC);

    $stmtC = $db->prepare("SELECT COUNT(*) as cant, COALESCE(SUM(total),0) as total FROM registro_compras WHERE empresa_id = ? AND DATE_FORMAT(fecha_emision,'%Y%m') = DATE_FORMAT(CURDATE(),'%Y%m')");
...
<div style="display:flex;align-items:center;gap:16px;margin-bottom:24px;">
    ...
    <div style="display:flex;gap:10px;">
        <a href="/empresas/<?= $empresa['id'] ?>/editar" style="background:var(--gc-surface-2);color:var(--gc-label);padding:8px 16px;border-radius:8px;font-size:13px;font-weight:600;text-decoration:none;">Editar</a>
        <a href="/empresas/<?= $empresa['id'] ?>/certificado" style="background:var(--gc-warn-soft);color:var(--gc-warn);padding:8px 16px;border-radius:8px;font-size:13px;font-weight:600;text-decoration:none;">Certificado SOL</a>
    </div>
</div>
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:24px;">
    <div style="background:var(--gc-surface);border-radius:12px;padding:18px;border:1px solid var(--gc-line);box-shadow:0 1px 4px rgba(0,0,0,0.05);">
        ...
        <div style="font-size:12px;color:var(--gc-muted);margin-top:4px;"><?= (int)($ultimasVentas['cant'] ?? 0) ?> comprobantes</div>
    </div>
    <div style="background:var(--gc-surface);border-radius:12px;padding:18px;border:1px solid var(--gc-line);box-shadow:0 1px 4px rgba(0,0,0,0.05);">
        ...
        <div style="font-size:12px;color:var(--gc-muted);margin-top:4px;"><?= (int)($ultimasCompras['cant'] ?? 0) ?> facturas</div>
    </div>
    <div style="background:var(--gc-surface);border-radius:12px;padding:18px;border:1px solid var(--gc-line);box-shadow:0 1px 4px rgba(0,0,0,0.05);">
        ...
        <?php if ($cert): ?>
            ...
            <div style="font-size:12px;color:var(--gc-muted);margin-top:4px;">Ambiente: <?= ucfirst($cert['ambiente'] ?? 'beta') ?></div>
        <?php else: ?>
...
<div style="background:var(--gc-surface);border-radius:12px;border:1px solid var(--gc-line);padding:40px;text-align:center;color:var(--gc-muted);font-size:14px;">
    ...
    Sin datos mensuales aún. Se generarán automáticamente cuando corran los crons de sincronización.
</div>
<?php endif; ?>
