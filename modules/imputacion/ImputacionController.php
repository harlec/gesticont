...
class ImputacionController
{
    public function index(int $empresaId): void
    {
        ...
        $stmtV = $pdo->prepare("
            ...
            WHERE empresa_id = ? AND estado_imputacion = 'pendiente' AND estado_sunat = '1'
        ");
        ...
        $pendientes = array_merge($stmtV->fetchAll(PDO::FETCH_ASSOC), $stmtC->fetchAll(PDO::FETCH_ASSOC));
        usort($pendientes, fn($a, $b) => $a['fecha_emision'] <=> $b['fecha_emision']);

        $tiposCompra = $pdo->query("
            SELECT id, nombre_visible FROM tipos_gasto
            WHERE aplica_a IN ('compra','ambos') AND activo = 1 ORDER BY orden
        ")->fetchAll(PDO::FETCH_ASSOC);

        $tiposVenta = $pdo->query("
            SELECT id, nombre_visible FROM tipos_gasto
            WHERE aplica_a IN ('venta','ambos') AND activo = 1 ORDER BY orden
        ")->fetchAll(PDO::FETCH_ASSOC);
